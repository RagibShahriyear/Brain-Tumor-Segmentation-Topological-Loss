...
def smooth_loss(pred, y, orig_y, weight_map, tumor):
    ...
    a = tf.where(
        valid1, gt[1:, :, :, :] * tf.abs(pred[1:, :, :, :] - pred[: h - 1, :, :, :]), z1
    )
    b = tf.where(
        valid2, gt[:, 1:, :, :] * tf.abs(pred[:, 1:, :, :] - pred[:, : w - 1, :, :]), z2
    )
    c = tf.where(
        valid3, gt[:, :, 1:, :] * tf.abs(pred[:, :, 1:, :] - pred[:, :, : d - 1, :]), z3
    )
...
def train(config_file):
    ...
    if net_name[6:8] == "TC":
        loss = (
            loss
            + 3 * topo_loss(proby, y, orig_y, w, "tc")
            + 3 * smooth_loss(proby, y, orig_y, w, "tc")
        )
    elif net_name[6:8] == "EN":
        loss = (
            loss
            + 3 * topo_loss(proby, y, orig_y, w, "en")
            + 3 * smooth_loss(proby, y, orig_y, w, "en")
        )
    ...
    for n in range(start_it, 40000):
        ...
        opt_step.run(
            session=sess, feed_dict={x: tempx, w: tempw, y: tempy, orig_y: orig_tempy}
        )
        ...
        if n % config_train["test_iteration"] == 0:
            ...
            for step in range(config_train["test_step"]):
                ...
                tempy = train_pair["labels"]
                dice = loss.eval(
                    feed_dict={x: tempx, w: tempw, y: tempy, orig_y: orig_tempy}
                )
